...
def parse_args():
    ...
    p.add_argument("--model", default="yolov12m.pt", help="Pretrained weights to fine-tune from.")
...
def parse_val_curve(results_csv: Path) -> list[dict]:
    ...
    Ultralytics opens results.csv in append mode, so re-training into an existing
    run dir (exist_ok=True) concatenates several runs into one file, each with the
    epoch counter restarting at 1. Keep only the most recent run -- the one that
    produced the current best.pt -- by resetting whenever the epoch number stops
    increasing. This prevents duplicate epoch entries in the results JSON.
    """
    curve = []
    ...
    with results_csv.open(newline="") as f:
        for row in csv.DictReader(f):
            ...
            if curve and epoch <= curve[-1]["epoch"]:
                curve = []  # a newer training run started; drop the stale rows
            entry = {"epoch": epoch}
...
def main():
    ...
    for ds in args.datasets:
        ...
        if args.skip_existing and results_csv.exists():
            ...
            # Ultralytics appends to results.csv (open mode "a"). With exist_ok=True
            # a stale CSV from a previous run would accumulate duplicate epochs, so
            # remove it to start this run's curve clean.
